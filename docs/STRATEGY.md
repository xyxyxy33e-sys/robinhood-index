...
4. Contract: nearest expiration ≥ `dte_target` (7) calendar days out, delta in
   [`target_delta_min`, `target_delta_max`] = [0.45, 0.55] via real greeks (this
   line previously carried stale 0DTE-era text — "today's expiration, delta
   0.35–0.45" — corrected 2026-08-07 to match `config/strategy.yaml`, which was
   always authoritative), premium ≥ `min_premium` ($0.15/share — cheaper
   contracts are spread-dominated; skip the entry rather than stepping strikes),
   bid/ask spread ≤ 10% of mid, open interest ≥ 250 (lowered from 500 on
   2026-08-07, owner decision — at 500 the filter consumed ~3 of ~11 genuine
   signals in the 7/24–8/7 forward period, including 2026-08-05's only
   qualifying signal; see `logs/analysis/2026-08-07_forward_review.md` finding
   5), volume ≥ 100.
...
| Exit | Mechanism |
|---|---|
| −28% static stop | Resting **stop-market, GTC** sell placed immediately after entry fill: trigger at 72% of fill (−28%, slippage margin). **This is the only stop an open position ever has — it is never moved** (the trailing stop was removed 2026-08-07, row below). It must be `gtc`, never `gfd`, or it expires at the close and leaves the position naked overnight. No limit floor: a stop-limit can gap through its floor and never fill. If the stop fills, re-entry is permitted under the normal re-entry rules below if the signal still qualifies. |
| ~~Trailing stop (post +10%)~~ | **REMOVED 2026-08-07 (owner decision — see `logs/analysis/2026-08-07_forward_review.md` finding 4).** Was a real, gating rule 2026-07-30 → 2026-08-07 (activation at +10% unrealized via `consider_exit_pct`, 14pp trail via `trail_stop_distance_pct`, both since deleted from config). It **never fired in the 8 forward trades** it was live or shadowed for: the 5 winners hit the +30% take-profit first every time, and the 3 losers never reached the +10% activation (peaks +2.4%, +3.6%, and one pre-dating the rule). Under a +30% TP it is mathematically near-dead code — a 14pp trail only locks in better than breakeven once peak gain exceeds +24%, and TP fires at +30%, so the protective band was a sliver. Any future re-parameterization (e.g. lower activation) requires a real backtest first: a tighter trail would likely have killed the 2026-07-30 winner during its −20% trough en route to +32%. `stops --peak` still reports `peak_gain_pct` as a diagnostic. |
| EOD carry gate (−14% at 15:45 ET) | **Added 2026-08-07 (owner decision) — REAL, gating rule.** At Phase 5 (15:45 ET), any open position whose mid is at or below `eod_carry_floor` = fill × (1 + `eod_carry_min_unrealized_pct`/100) (config, −14% = half the stop width) is CLOSED before 16:00 (cancel stop, bid-pegged limit) instead of carried overnight. Rationale: that close to the −28% stop there is under half the stop width left as cushion, and overnight gaps fill stops far through their trigger — June data showed fills at −56/−58% vs the nominal −28% (`logs/backtest/take_profit_30pct_test.md`; 27% of stop fills meaningfully off-trigger). Positions above the floor carry as designed with the GTC stop. **Zero backtest evidence for −14 specifically** — an unvalidated starting guess adopted before the first-ever overnight carry (none of the 8 forward trades has yet carried) so the first carry decision is principled rather than improvised. Journal the gate decision every day a position is open at 15:45. |
