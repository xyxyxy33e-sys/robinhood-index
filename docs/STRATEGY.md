...
| +30% take-profit | Checked every minute during market hours. If mid ≥ 130% of fill: cancel the stop, sell at bid-pegged limit, then confirm flat. (Not resting — Robinhood allows only one working sell against a position.) Lowered from +60% on 2026-07-29 after a SPY 740P round-tripped from +40.6% unrealized (12:15 ET) to a −28.96% stop-out (14:45 ET) on an FOMC-driven reversal — see `logs/journal/2026-07-29.md`. The +60% level came from the July 0DTE backtest (median MFE only +13.4% at 7DTE, target reached 2/19 times — see `logs/backtest/dte_comparison.md`); +30% has not itself been backtested yet. |
...
| *(diagnostic)* Consider-exit checkpoint | **Not an exit.** Added 2026-07-29 alongside the take-profit change above: log every minute once unrealized gain first crosses `consider_exit_pct` (default +10%, `strategy_calc.py stops` → `consider_exit`), same "record, never act" status as signal decay. Zero backtest evidence — this is a cheap early flag, not a validated threshold, in case a future rule wants to act before +30%. |
